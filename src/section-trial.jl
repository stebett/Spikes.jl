...
function section_trial(r::Vector{T}, ranges::Vector{<:Tuple}, bins::Vector{T}, 
	...
	section_trial(r, x, ranges, bins)
	zscore!(r, mean(r[1:floor(Int, length(r)÷4)]), std(r[1:floor(Int, length(r)÷4)]))
end
	
function section_trial(r::Vector{Vector{T}}, ranges::Vector{<:Vector{<:Tuple}}, bins::Vector{T}, row, pad::Int, n::Int, b1::Int) where {T <: Real}
	section_trial.(r, ranges, Ref(bins), Ref(row.t), row.lift, row.cover, row.grasp, pad, n, b1)
end

function section_trial(r::Vector{Vector{Vector{T}}}, ranges::Vector{<:Vector{<:Vector{<:Tuple}}}, bins::Vector{T}, df, pad::Int, n::Int, b1::Int) where {T <: Real}
	section_trial.(r, ranges, Ref(bins), eachrow(df), pad, n, b1)
end
															 
function section_trial(df, pad::Int, n::Int, b1::Int)
	...
	section_trial(r, ranges, bins, df, pad, n, b1)
	return r, ranges
end
